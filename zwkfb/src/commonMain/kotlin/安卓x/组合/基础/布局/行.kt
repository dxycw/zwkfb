...
@LayoutScopeMarker
@Immutable
interface 行范围 { // RowScope
    /**
     * 根据该元素在 [Row] 中与其他加权同级元素的相对 [weight] 按比例设置元素宽度。父布局会在测量完未加权的子元素后，
     * 将剩余的横向空间按照此权重进行分配。当 [fill] 为 true 时，该元素将被强制占据分配到的全部宽度；否则，该元素允许
     * 更小——这将导致 [Row] 本身也变小，因为未使用的分配宽度不会重新分配给其他同级元素。
     *
     * @param 权重 赋予该元素的相对宽度比例，相对于所有加权同级元素的总权重而言。必须为正数。
     ...
     * @see 按基线对齐
     */
    @Stable fun Modifier.对齐方式(对齐线: HorizontalAlignmentLine): Modifier
     ...
     * @see 对齐方式
     */
    @Stable fun Modifier.按基线对齐(): Modifier
     ...
     */
    @Stable fun Modifier.对齐方式(对齐线块: (Measured) -> Int): Modifier
}
