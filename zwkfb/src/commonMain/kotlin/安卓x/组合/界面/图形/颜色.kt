...
/**
 * 在两种 [颜色][Color]（[起始] 和 [停止]）之间按 [比例] 比例进行线性插值。结果颜色的 [ColorSpace] 始终为
 * [停止] 的 [ColorSpace][Color.colorSpace]。[比例] 应在 0 到 1 之间（含边界）。插值在 [ColorSpaces.Oklab]
 * 颜色空间中进行。
 */
@Stable
fun 线性插值(起始: Color, 停止: Color, @FloatRange(from = 0.0, to = 1.0) 比例: Float): Color =
    lerp(start = 起始, stop = 停止, fraction = 比例)
