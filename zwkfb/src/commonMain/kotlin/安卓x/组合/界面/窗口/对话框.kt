 ...
 * 用于自定义 [对话框] 行为的属性。
 *
 * @param 关闭返回键回调 指定在 Android 上按下返回键，或在桌面端按下 Esc 键时，是否可以关闭该弹窗。
 * 如果为 true，按下返回键将触发 onDismissRequest 回调。
 * @param 关闭单击外部回调 指定点击对话框外部区域时，是否可以关闭该对话框。如果为 `true`，
...
val DialogProperties.关闭返回键回调: Boolean
    get() = this.dismissOnBackPress

val DialogProperties.关闭单击外部回调: Boolean
    get() = this.dismissOnClickOutside

val DialogProperties.使用平台默认宽度: Boolean
    get() = this.usePlatformDefaultWidth
