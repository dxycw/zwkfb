...
import 安卓x.组合.材质3.令牌集.SliderTokens
...
object 滑块默认值 { // SliderDefaults
    ...
    @Composable fun 颜色集() = SliderDefaults.colors()
...
val SliderColors.滑块颜色: Color
    get() = this.thumbColor

val SliderColors.激活轨道颜色: Color
    get() = this.activeTrackColor

val SliderColors.激活刻度颜色: Color
    get() = this.activeTickColor

val SliderColors.非激活轨道颜: Color
    get() = this.inactiveTrackColor

val SliderColors.非激活刻度颜色: Color
    get() = this.inactiveTickColor

val SliderColors.禁用滑块颜色: Color
    get() = this.disabledThumbColor

val SliderColors.禁用激活轨道颜色: Color
    get() = this.disabledActiveTrackColor

val SliderColors.禁用激活刻度颜色: Color
    get() = this.disabledActiveTickColor

val SliderColors.禁用非激活轨道颜色: Color
    get() = this.disabledInactiveTrackColor

val SliderColors.禁用非激活刻度颜色: Color
    get() = this.disabledInactiveTickColor
...
fun 滑块位置集(
    ...
    SliderPositions(
        initialActiveRange = 初始激活范围,
        initialTickFractions = 初始刻度比例,
    )

//========================================================================================

/**
 * 表示当前激活范围的 [ClosedFloatingPointRange]，对于 [Slider] 是从起始位置到滑钮，对于 [RangeSlider]
 * 是从起始滑钮到结束滑钮。
 */
val SliderPositions.激活范围: ClosedFloatingPointRange<Float>
    get() = this.activeRange


/**
 * 刻度应在轨道上绘制的离散点。刻度比例 的每个值应在 [0f, 1f] 范围内。如果轨道是连续的，则 tickFractions
 * 将为空的 [FloatArray]。
 */
val SliderPositions.刻度比例: FloatArray
    get() = this.tickFractions
...
@get:IntRange(from = 0)
val SliderState.步数: Int
    get() = this.steps

var SliderState.值改变完成回调: (() -> Unit)?
    get() = this.onValueChangeFinished
    set(value) {
        this.onValueChangeFinished = value
    }

val SliderState.值范围: ClosedFloatingPointRange<Float>
    get() = this.valueRange

/** 表示滑钮当前相对于轨道所处位置的值的 [Float]。 */
var SliderState.值: Float
    get() = this.value
    set(newVal) {
        this.value = newVal
    }
...
var SliderState.是否自动吸附: Boolean
    get() = this.shouldAutoSnap
    set(value) {
...
val SliderState.是否正在拖动: Boolean
    get() = this.isDragging
...
@get:IntRange(from = 0)
val RangeSliderState.步数: Int
    get() = this.steps

var RangeSliderState.值改变完成回调: (() -> Unit)?
    get() = this.onValueChangeFinished
    set(newVal) {
        this.onValueChangeFinished = newVal
    }

val RangeSliderState.值范围: ClosedFloatingPointRange<Float>
    get() = this.valueRange


/** 表示 [RangeSlider] 当前激活范围起始位置的 [Float]。 */
var RangeSliderState.激活范围开始: Float
    get() = this.activeRangeStart
    set(newVal) {
        this.activeRangeStart = newVal
    }

/** 表示 [RangeSlider] 当前激活范围结束位置的 [Float]。 */
var RangeSliderState.激活范围结束: Float
    get() = this.activeRangeEnd
    set(newVal) {
        this.activeRangeEnd = newVal
    }
