...
object 文本字段默认值 { // TextFieldDefaults
    ...
    /** 根据 Expressive 风格推荐的 [文本字段] 圆角形状。 */
    val 圆角形状: Shape
        // TODO(b/448727879): 在令牌就位后引用实际的令牌。
        @Composable get() = TextFieldDefaults.roundedShape
    ...
    fun 内容内边距带标签(
        开始: Dp = 16.dp,
        结束: Dp = 16.dp,
        上: Dp = TextFieldWithLabelVerticalPadding,
        下: Dp = TextFieldWithLabelVerticalPadding,
    ): PaddingValues =
        TextFieldDefaults.contentPaddingWithLabel(
    ...
    internal fun 默认内容内边距(
        标签: @Composable (TextFieldLabelScope.() -> Unit)?,
        标签位置: TextFieldLabelPosition,
    ): PaddingValues =
        if (标签 != null && 标签位置.normalize() is TextFieldLabelPosition.Inside) {
            TextFieldDefaults.contentPaddingWithLabel()
        } else {
            TextFieldDefaults.contentPaddingWithoutLabel()
        }
    ...
    /**
     * 默认应用于 [文本字段] 和 [轮廓文本字段] 辅助文本的内边距。详见 [PaddingValues]。
     */
    // TODO(246775477): 考虑把这个公开
    ...
    /**
     * 创建一个 [TextFieldColors]，表示在采用色调颜色的 [文本字段] 中使用的 Expressive 风格输入文本、容器和
     * 内容颜色（包括标签、占位符、图标等）。
     *
     * 应与 [TextFieldLabelPosition.Inside] 配合使用。
     */
    @Composable
    fun 色调颜色集() = TextFieldDefaults.tonalColors()
...
object 轮廓文本字段默认值 { // OutlinedTextFieldDefaults
    ...
    /** Expressive 风格推荐的、用于 [轮廓文本字段] 的圆角形状。 */
    val 圆角形状: Shape
        // TODO(b/448727879): 在实际 token 就位后再引用它
        @Composable get() = OutlinedTextFieldDefaults.roundedShape
    ...
    fun 装饰器(
        ...
        输出转换: OutputTransformation?,
        交互源: InteractionSource,
        标签位置: TextFieldLabelPosition = TextFieldLabelPosition.Cutout(),
        标签: @Composable (TextFieldLabelScope.() -> Unit)? = null,
        占位符: @Composable (() -> Unit)? = null,
        前导图标: @Composable (() -> Unit)? = null,
        尾随图标: @Composable (() -> Unit)? = null,
        ...
        内容内边距: PaddingValues = 轮廓文本字段默认值.默认内容内边距(标签, 标签位置),
        容器: @Composable () -> Unit = {
    ...
    fun 装饰盒子(
        ...
        辅助文本: @Composable (() -> Unit)? = null,
        颜色集: TextFieldColors = OutlinedTextFieldDefaults.colors(),
        内容内边距: PaddingValues = OutlinedTextFieldDefaults.contentPaddingWithoutLabel(),
        容器: @Composable () -> Unit = {
            OutlinedTextFieldDefaults.Container(
                enabled = 已启用,
                isError = 是否错误,
    ...
    /**
     * [OutlinedTextField] 中输入框的默认内容内边距，适用于存在内置标签的情况。请注意，顶部内边距表示聚焦状态下标签上方的间距。
     * 输入框直接位于标签下方。
     *
     * 水平内边距表示输入框与前置/后置图标（若存在）之间的间距；若无图标，则表示输入框与容器左右边缘之间的间距。
     */
    fun 内容内边距带标签(
        开始: Dp = 16.dp,
        上: Dp = TextFieldWithLabelVerticalPadding,
    ...
    /**
     * [OutlinedTextField] 中输入框的默认内容内边距，适用于标签不存在或未置于容器内部的情况。
     *
     * 水平内边距表示输入框与前置/后置图标（若存在）之间的间距；若无图标，则表示输入框与容器左右边缘之间的间距。
     */
    fun 内容内边距带无标签(
    ...
    /** 根据 [标签] 是否存在及其 [标签位置]，确定 [TextField] 中输入框的默认内容内边距。*/
    internal fun 默认内容内边距(
        标签: @Composable (TextFieldLabelScope.() -> Unit)?,
        标签位置: TextFieldLabelPosition,
    ): PaddingValues =
        if (标签 != null && 标签位置 is TextFieldLabelPosition.Inside) {
            OutlinedTextFieldDefaults.contentPaddingWithLabel()
        } else {
            OutlinedTextFieldDefaults.contentPaddingWithoutLabel()
        }
    ...
    @Composable fun 颜色集() = OutlinedTextFieldDefaults.colors()

    /**
     * 创建一个 [TextFieldColors]，表示用于 [OutlinedTextField] 的 Expressive 风格输入文本、容器及内容颜色
     * （包括标签、占位符、图标等），采用色调配色方案。
     *
     * 此属性应与 [TextFieldLabelPosition.Inside] 配合使用。
     */
    @Composable
    fun 色调颜色集() = OutlinedTextFieldDefaults.tonalColors()
...
val TextFieldColors.聚焦文本颜色: Color
    get() = this.focusedTextColor
val TextFieldColors.未聚焦文本颜色: Color
    get() = this.unfocusedTextColor
val TextFieldColors.禁用文本颜色: Color
    get() = this.disabledTextColor
val TextFieldColors.错误文本颜色: Color
    get() = this.errorTextColor
val TextFieldColors.聚焦容器颜色: Color
    get() = this.focusedContainerColor
val TextFieldColors.未聚焦容器颜色: Color
    get() = this.unfocusedContainerColor
val TextFieldColors.禁用容器颜色: Color
    get() = this.disabledContainerColor
val TextFieldColors.错误容器颜色: Color
    get() = this.errorContainerColor
val TextFieldColors.光标颜色: Color
    get() = this.cursorColor
val TextFieldColors.错误光标颜色: Color
    get() = this.errorCursorColor
val TextFieldColors.文本选择颜色集: TextSelectionColors
    get() = this.textSelectionColors
val TextFieldColors.聚焦指示器颜色: Color
    get() = this.focusedIndicatorColor
val TextFieldColors.未聚焦指示器颜色: Color
    get() = this.unfocusedIndicatorColor
val TextFieldColors.禁用指示器颜色: Color
    get() = this.disabledIndicatorColor
val TextFieldColors.错误指示器颜色: Color
    get() = this.errorIndicatorColor
val TextFieldColors.聚焦前导图标颜色: Color
    get() = this.focusedLeadingIconColor
val TextFieldColors.未聚焦前导图标颜色: Color
    get() = this.unfocusedLeadingIconColor
val TextFieldColors.禁用前导图标颜色: Color
    get() = this.disabledLeadingIconColor
val TextFieldColors.错误前导图标颜色: Color
    get() = this.errorLeadingIconColor
val TextFieldColors.聚焦尾随图标颜色: Color
    get() = this.focusedTrailingIconColor
val TextFieldColors.未聚焦尾随图标颜色: Color
    get() = this.unfocusedTrailingIconColor
val TextFieldColors.禁用尾随图标颜色: Color
    get() = this.disabledTrailingIconColor
val TextFieldColors.错误尾随图标颜色: Color
    get() = this.errorTrailingIconColor
val TextFieldColors.聚焦标签颜色: Color
    get() = this.focusedLabelColor
val TextFieldColors.未聚焦标签颜色: Color
    get() = this.unfocusedLabelColor
val TextFieldColors.禁用标签颜色: Color
    get() = this.disabledLabelColor
val TextFieldColors.错误标签颜色: Color
    get() = this.errorLabelColor
val TextFieldColors.聚焦占位符颜色: Color
    get() = this.focusedPlaceholderColor
val TextFieldColors.未聚焦占位符颜色: Color
    get() = this.unfocusedPlaceholderColor
val TextFieldColors.禁用占位符颜色: Color
    get() = this.disabledPlaceholderColor
val TextFieldColors.错误占位符颜色: Color
    get() = this.errorPlaceholderColor
val TextFieldColors.聚焦辅助文本颜色: Color
    get() = this.focusedSupportingTextColor
val TextFieldColors.未聚焦辅助文本颜色: Color
    get() = this.unfocusedSupportingTextColor
val TextFieldColors.禁用辅助文本颜色: Color
    get() = this.disabledSupportingTextColor
val TextFieldColors.错误辅助文本颜色: Color
    get() = this.errorSupportingTextColor
val TextFieldColors.聚焦前缀颜色: Color
    get() = this.focusedPrefixColor
val TextFieldColors.未聚焦前缀颜色: Color
    get() = this.unfocusedPrefixColor
val TextFieldColors.禁用前缀颜色: Color
    get() = this.disabledPrefixColor
val TextFieldColors.错误前缀颜色: Color
    get() = this.errorPrefixColor
val TextFieldColors.聚焦后缀颜色: Color
    get() = this.focusedSuffixColor
val TextFieldColors.未聚焦后缀颜色: Color
    get() = this.unfocusedSuffixColor
val TextFieldColors.禁用后缀颜色: Color
    get() = this.disabledSuffixColor
val TextFieldColors.错误后缀颜色: Color
    get() = this.errorSuffixColor
...
object 文本字段标签位置{ // TextFieldLabelPosition
     ...
     * 这是 [文本字段] 的默认标签位置。
     *
     * @param 始终最小化  是否始终保持文本字段标签最小化。如果为 `false`，当文本字段未获得焦点且为空时，
     * 标签将展开以占据输入区域。如果为 `true`，则允许在文本字段未获得焦点且为空时，将占位符、前缀和后缀与标签一并显示。
     * @param 最小化对齐方式 标签最小化时的水平对齐方式。
     * @param 展开对齐方式 标签展开时的水平对齐方式。
     */
    fun 里面(
        始终最小化: Boolean = false,
        最小化对齐方式: Alignment.Horizontal = Alignment.Start,
        ...
        TextFieldLabelPosition.Inside(
            isAlwaysMinimized = 始终最小化,
            minimizedAlignment = 最小化对齐方式,
            expandedAlignment = 展开对齐方式
        )

    val TextFieldLabelPosition.Inside.始终最小化: Boolean
        get() = this.isAlwaysMinimized
    val TextFieldLabelPosition.Inside.最小化对齐方式: Alignment.Horizontal
        get() = this.minimizedAlignment
    val TextFieldLabelPosition.Inside.展开对齐方式: Alignment.Horizontal
        get() = this.expandedAlignment

    /**
     *  标签在展开时位于文本字段容器内部，在最小化时切入边框。
     *
     * 这是 [轮廓文本字段] 的默认标签位置。
     *
     * @param 始终最小化  是否始终保持文本字段标签最小化。如果为 `false`，当文本字段未获得焦点且为空时，
    ...
    val TextFieldLabelPosition.Cutout.始终最小化: Boolean
        get() = this.isAlwaysMinimized
    val TextFieldLabelPosition.Cutout.最小化对齐方式: Alignment.Horizontal
        get() = this.minimizedAlignment
    val TextFieldLabelPosition.Cutout.展开对齐方式: Alignment.Horizontal
        get() = this.expandedAlignment
 ...
 * 此 [标签最小化进度] 值可用于与默认动画协调配合其他动画。
 */
@get:FloatRange(from = 0.0, to = 1.0) val TextFieldLabelScope.标签最小化进度: Float
