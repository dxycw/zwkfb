 ...
 *
 * 此外，对于[颜色]，如果[颜色]未设置，且[样式]也没有颜色，则将使用[LocalContentColor]。
 *
 * @param 文本 要显示的文本。
 * @param 修饰符 要应用于此布局节点的 [Modifier]。
 * @param 颜色 要应用于文本的 [Color]。如果为 [Color.Unspecified]，且 [样式] 未设置颜色，则将使用 [LocalContentColor]。
 * @param 自适应大小 为此文本可组合项启用自动调整大小功能。查找适合可用空间的最大字体大小，并使用该大小进行文本布局。
 * 这会执行多次布局遍历，可能比使用固定字体大小更慢。此功能优先于通过 [字体大小] 和 [样式] 定义的大小。请参阅 [TextAutoSize]。
 ...
 * @param 最大行数 文本可跨越的最大行数，必要时自动换行。如果文本超出给定的行数，将根据 [溢出] 和 [软换行] 进行截断。
 * 要求满足 1 <= [最小行数] <= [最大行数]。
...
/**
 * 包含首选 [TextStyle] 的 CompositionLocal，该样式将被 [Text] 组件默认使用。要设置此 CompositionLocal 的值，
 * 请参阅 [ProvideTextStyle]，它会将任何缺失的 [TextStyle] 属性与当前已设置在此 CompositionLocal 中的现有 [TextStyle] 进行合并。
 *
...
// TODO(b/156598010): 移除此处内容，并在底层的 CompositionLocal 上替换为 fold 定义。
/**
 * 此函数用于设置 [LocalTextStyle] 的当前值，将给定样式与当前样式值合并，以填充任何缺失的属性。此组件的 [内容]
 * 中包含的任何 [Text] 组件都将应用此样式，除非被显式设置样式覆盖。
 *
 * @see LocalTextStyle
 */
@Suppress("ComposableNaming")
