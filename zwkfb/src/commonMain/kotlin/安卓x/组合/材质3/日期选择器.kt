...
interface 日期选择器状态 { // DatePickerState
     ...
     */
    val 可选日期: SelectableDates

    /** 一个语言环境，用于格式化日期、确定输入格式、星期名称显示等。*/
    val 语言环境: CalendarLocale
 ...
 */
@get:Suppress("AutoBoxing")
var DatePickerState.已选择日期毫秒: Long?
...
fun DatePickerFormatter.格式化日期(
    ...
    语言环境: CalendarLocale,
    用于内容描述: Boolean = false,
): String? = this.formatDate(dateMillis = 日期毫秒时间戳, locale = 语言环境, forContentDescription = 用于内容描述)
...
object 日期选择器默认值 { // DatePickerDefaults
    ...
    @Composable
    fun 颜色集() = DatePickerDefaults.colors()
