...
object 顶部应用栏默认值 { // TopAppBarDefaults
    ...
    @Composable
    fun 顶部应用栏颜色集() = TopAppBarDefaults.topAppBarColors()
     ...
     *
     * @param 滚动状态 滚动状态
     * @param 反转滚动 反转滚动方向，当为 true 时，0 [ScrollState.value] 表示底部；当为 false 时，
    ...
    fun 进入始终滚动行为(
        状态: TopAppBarState = rememberTopAppBarState(),
        可以滚动: () -> Boolean = { true },
        吸附动画规格: AnimationSpec<Float>? = MaterialTheme.motionScheme.defaultEffectsSpec(),
...
object 底部应用栏默认值 { // BottomAppBarDefaults
    ...
    // TODO: 从组件令牌文件加载 motionScheme 令牌
    /**
