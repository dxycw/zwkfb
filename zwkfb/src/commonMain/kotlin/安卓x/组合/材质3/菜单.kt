 ...
 *
 * 此可组合项提供了菜单的 [Popup] 和布局行为。这对于构建需要与默认 [DropdownMenu] 不同的内容排列或样式的自定义菜单非常有用。
 *
 * @param 已展开 菜单是否展开。
 ...
 * [Material Design dropdown menu](https://m3.material.io/components/menus/overview)
 *
 * 用于在 [下拉菜单弹出] 中创建视觉上有明显区分的内容组的可组合项。
 *
...
val MenuItemColors.文本颜色: Color
    get() = this.textColor

val MenuItemColors.前导图标颜色: Color
    get() = this.leadingIconColor

val MenuItemColors.尾随图标颜色: Color
    get() = this.trailingIconColor

val MenuItemColors.禁用文本颜色: Color
    get() = this.disabledTextColor

val MenuItemColors.禁用前导图标颜色: Color
    get() = this.disabledLeadingIconColor

val MenuItemColors.禁用尾随图标颜色: Color
...
val MenuItemShapes.形状: Shape
    get() = this.shape

val MenuItemShapes.已选择形状: Shape
...
val MenuGroupShapes.形状: Shape
    get() = this.shape

val MenuGroupShapes.非激活形状: Shape
    get() = this.inactiveShape

//======================================================================


/** 提供用于计算相对于窗口边界的候选菜单位置坐标的上下文。*/
interface 菜单位置范围 { // MenuPositionScope

    /** 锚点相对于窗口布局边界的边界。 */
    val 锚点边界: IntRect // anchorBounds

    /** 承载窗口的整体大小。 */
    val 窗口大小: IntSize // windowSize

    /** 菜单弹出层计算后的尺寸。 */
    val 菜单大小: IntSize // menuSize
...
/** 锚点相对于窗口布局边界的边界。 */
val MenuPositionScope.锚点边界: IntRect
    get() = this.anchorBounds

/** 承载窗口的整体大小。 */
val MenuPositionScope.窗口大小: IntSize
    get() = this.windowSize

/** 菜单弹出层计算后的尺寸。 */
val MenuPositionScope.菜单大小: IntSize
    get() = this.menuSize

/** 当前活动的布局方向（从左到右或从右到左）。 */
val MenuPositionScope.布局方向: LayoutDirection
 ...
 * 确定菜单相对于其锚点位置的类。
 *
 * 这允许在标准定位策略（如 [上方]、[下方]、[开始]、[结束]、[左侧]、[右侧]）之间进行选择，
 * 或为复杂的定位逻辑提供 [自定义] 实现。
 */
object 菜单锚点位置{
    /**
     * 将菜单定位在其锚点上方。
     *
     * 菜单的底部边缘默认与锚点的顶部边缘对齐。如果空间不足，将尝试替代位置（例如锚点下方）。
     */
    val 上方 = MenuAnchorPosition.Above

    /**
     * 将菜单定位在其锚点下方。
     *
     * 菜单的顶部边缘默认与锚点的底部边缘对齐。如果空间不足，将尝试替代位置（例如锚点上方）。
     */
    val 下方 = MenuAnchorPosition.Below


    /**
     * 将菜单定位在其锚点的左侧。
     *
     * 此策略将菜单定位在左侧，不受布局方向的影响。
     */
    val 左侧 = MenuAnchorPosition.Left

    /**
     * 将菜单定位在其锚点的右侧。
     *
     * 此策略将菜单定位在右侧，不受布局方向的影响。
     */
    val 右侧 = MenuAnchorPosition.Right

    /**
     * 将菜单定位在其锚点的起始位置。
     *
     * 在从左到右布局中，这将菜单定位在锚点的左侧。在从右到左布局中，这将菜单定位在锚点的右侧。
     */
    val 开始 = MenuAnchorPosition.Start

    /**
     * 将菜单定位在其锚点的末尾位置。
     *
     * 在从左到右布局中，这将菜单定位在锚点的右侧。在从右到左布局中，这将菜单定位在锚点的左侧。
     */
    val 结束 = MenuAnchorPosition.End
...
interface 下拉菜单弹出位置提供器 : PopupPositionProvider { // DropdownMenuPopupPositionProvider
    ...
    /**
     * 下拉菜单弹出层相对于其锚点计算得出的 [TransformOrigin]。
     *
     * 此原点用于从菜单定位位置的正确点来动画化（例如缩放）菜单。
     */
    val 变换原点: TransformOrigin // transformOrigin
 ...
 * 此原点用于从菜单定位位置的正确点来动画化（例如缩放）菜单。
 */
val DropdownMenuPopupPositionProvider.变换原点: TransformOrigin
