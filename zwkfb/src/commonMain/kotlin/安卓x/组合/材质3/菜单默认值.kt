...
object 菜单默认值 { // MenuDefaults
    ...
    @Composable fun 项颜色集(): MenuItemColors = MenuDefaults.itemColors()
     ...
     * @param 已选择形状 选中时的形状。如果传入 null，则默认使用 [已选择形状]。
     */
    @Composable
    fun 项形状集(形状: Shape? = null, 已选择形状: Shape? = null): MenuItemShapes =
        MenuDefaults.itemShapes(shape = 形状, selectedShape = 已选择形状,)
     ...
     */
    @Composable
    fun 组形状集(): MenuGroupShapes = MenuDefaults.groupShapes()
    ...
    val 下拉菜单项尾随标签水平内边距
        get() = MenuDefaults.DropdownMenuItemTrailingLabelHorizontalPadding
    ...
    /**
     * 创建并记住一个 [DropdownMenuPopupPositionProvider]，用于将下拉菜单相对于其锚点进行定位。
     *
     * @param 下拉菜单锚点位置 要使用的定位策略。这决定了菜单相对于锚点的首选对齐方式。有预定义的位置，请参阅
     * [MenuAnchorPosition.Above]、[MenuAnchorPosition.Below]、[MenuAnchorPosition.Left]、
     * [MenuAnchorPosition.Right]、[MenuAnchorPosition.Start] 和 [MenuAnchorPosition.End]。
     * 也可以通过使用 [MenuAnchorPosition.Custom] 来定义自定义定位。
     * @param 偏移量 一个可选的 [DpOffset]，用于应用到最终计算出的位置上。
     */
    @Composable
    fun 记住下拉菜单弹出位置提供器(
        下拉菜单锚点位置: MenuAnchorPosition,
        偏移量: DpOffset = DpOffset(0.dp, 0.dp),
    ): DropdownMenuPopupPositionProvider =
        rememberDropdownMenuPopupPositionProvider(
            dropdownMenuAnchorPosition = 下拉菜单锚点位置,
            offset = 偏移量,
        )


    /** 菜单项的默认水平排列方式。 */
    val 下拉菜单项水平排列: Arrangement.Horizontal
