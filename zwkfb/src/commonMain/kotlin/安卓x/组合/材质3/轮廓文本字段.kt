 ...
 * 轮廓式文本字段比填充式文本字段的视觉强调效果更弱。当它们出现在表单等许多文本字段集中放置的地方时，其较低的强调效果有助于简化布局。
 *
 ...
 *
 * 如果你需要填充式版本，请查看 [文本字段]。如需专门用于密码或其他安全内容的文本字段，请查看 [OutlinedSecureTextField]。
 *
 * 这个 [轮廓文本字段] 的重载版本使用 [TextFieldState] 来跟踪其文本内容以及光标或选区的位置。
 *
 ...
 * @param 滚动状态 管理文本字段水平或垂直滚动的滚动状态。如果 [行限制] 为 [SingleLine]，则该文本字段被视为单行，具有水平滚动行为；
 * 否则，文本字段将变为垂直可滚动。
 * @param 形状 定义此文本字段边框的形状。
 * @param 颜色集 用于解析此文本字段在不同状态下所使用的颜色的 [TextFieldColors]。参见 [OutlinedTextFieldDefaults.colors]。
 * @param 内容内边距 应用于内部文本字段的内边距，用于将其与文本字段周围的其他元素分隔开。请注意，如果这些内边距值与文本字段的尺寸约束或布局不兼容，
 * 则可能不会被采用。参见 [OutlinedTextFieldDefaults.contentPadding]。
 * @param 交互源 可选的提升式 [MutableInteractionSource]，用于观察和发送此文本字段的 [Interaction]。你可以使用它来更改文本字段的外观，
 * 或在不同状态下预览文本字段。请注意，即使传入 `null`，交互仍会在内部发生。
 */
 ...
 * ![Outlined text field image](https://developer.android.com/images/reference/androidx/compose/material3/outlined-text-field.png)
 *
 * 如果你除了输入文本的变化外，还想观察光标位置、选区范围或输入法组合状态，请改用带有 [TextFieldValue] 参数的 OutlinedTextField 重载版本。
 *
 ...
 * ![Outlined text field image](https://developer.android.com/images/reference/androidx/compose/material3/outlined-text-field.png)
 *
 * 这个重载版本提供了对输入文本、光标位置、选区范围和输入法组合状态的访问。如果你只想观察输入文本的变化，请改用带有 [String] 参数的 轮廓文本字段 重载版本。
 *
