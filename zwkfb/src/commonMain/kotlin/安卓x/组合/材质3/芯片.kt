...
import 安卓x.组合.材质3.令牌集.AssistChipTokens
import 安卓x.组合.材质3.令牌集.FilterChipTokens
import 安卓x.组合.材质3.令牌集.InputChipTokens
import 安卓x.组合.材质3.令牌集.SuggestionChipTokens
 ...
 * [Material Design filter chip](https://m3.material.io/components/chips/overview)
 *
 * Chips 帮助用户输入信息、做出选择、过滤内容或触发操作。Chips 可以在同一区域中展示多个交互元素，例如可选的电影场次列表，或一系列邮件联系人。
 *
 * Filter chips 使用标签或描述性文字来过滤内容。它们可以作为切换按钮或复选框的良好替代方案。只要提供的形状是 [CornerBasedShape]，
 * 它还会根据 chip 的状态在 [形状集] 中提供的形状之间进行变形过渡。如果 [形状集] 中的某个形状不是 [CornerBasedShape]，
 * 那么 chip 将根据用户交互在 [ChipShapes] 之间切换。
 *
 * ![Filter chip image](https://developer.android.com/images/reference/androidx/compose/material3/filter-chip-with-corner-morphing.png)
 *
 * 此筛选 Chip 应用了扁平样式。如需凸起样式，请使用 [ElevatedFilterChip]。
 *
 * 点击筛选 Chip 会切换其选中状态。可以提供一个选中状态的 [前导图标]（例如对勾标记），显示在 Chip 标签的起始边缘。
 *
 * @param 已选择 此 Chip 是否被选中。
 * @param 单击回调 当此 Chip 被点击时调用。
 * @param 标签 此 Chip 的文本标签。
 * @param 形状集 此 Chip 根据其选中状态在之间变形的 [ChipShapes]。
 * @param 修饰符 要应用于此 Chip 的 [Modifier]。
 * @param 已启用 控制此 Chip 的启用状态。当为 `false` 时，该组件不会响应用户输入，并且在外观上显示为禁用状态，同时无障碍服务也会将其识别为禁用。
 * @param 前导图标 Chip 起始位置的可选图标，位于标签文本之前。当已选择为 true 时，该图标可以视觉上指示 Chip 已被选中（例如，通过对勾图标）。
 * @param 尾随图标 Chip 末尾的可选图标。
 * @param 颜色集 用于解析此 Chip 在不同状态下所用颜色的 [SelectableChipColors]。参见 [FilterChipDefaults.filterChipColors]。
 * @param 阴影 用于解析此 Chip 在不同状态下海拔高度的 [SelectableChipElevation]。这控制 Chip 下方阴影的大小。此外，
 * 当容器颜色为 [ColorScheme.surface] 时，这控制作为主色叠加应用的数量。参见 [FilterChipDefaults.filterChipElevation]。
 * @param 边框 绘制在此 Chip 容器周围的边框。传入 `null` 表示无边框。参见 [FilterChipDefaults.filterChipBorder]。
 * @param 水平排列 Chip 子元素的横向排列方式。如果没有图标，则标签与边框之间的水平内边距将为内容内边距与此水平排列中间距的总和。
 * @param 内容内边距 此 Chip 内容周围的内边距，包括前导图标、标签和尾随图标。
 * @param 交互源 一个可选的提升式 [MutableInteractionSource]，用于观察和发射此 Chip 的 [Interaction]。
...
fun 过滤芯片(
    ...
    前导图标: @Composable (() -> Unit)? = null,
    尾随图标: @Composable (() -> Unit)? = null,
    颜色集: SelectableChipColors = FilterChipDefaults.tonalFilterChipColors(),
    阴影: SelectableChipElevation? = FilterChipDefaults.filterChipElevation(),
    边框: BorderStroke? = FilterChipDefaults.filterChipBorder(已启用, 已选择),
    水平排列: Arrangement.Horizontal =
        FilterChipDefaults.horizontalArrangement(前导图标 != null, 尾随图标 != null),
    内容内边距: PaddingValues = FilterChipDefaults.ContentPadding,
    交互源: MutableInteractionSource? = null,
) =
    FilterChip(
 ...
 * [Material Design elevated filter chip](https://m3.material.io/components/chips/overview)
 *
 * Chips 帮助用户输入信息、做出选择、过滤内容或触发操作。Chips 可以在同一区域中展示多个交互元素，例如可选的电影场次列表，
 * 或一系列邮件联系人。
 *
 * 筛选 Chip 使用标签或描述性文字来过滤内容。它们可以作为切换按钮或复选框的良好替代方案。只要提供的形状是 [CornerBasedShape]，
 * 它还会根据 Chip 的状态在 [形状集] 中提供的形状之间进行变形过渡。如果 [形状集] 中的某个形状不是 [CornerBasedShape]，
 * 那么 Chip 将根据用户交互在 [ChipShapes] 之间切换。
 *
 * ![Filter chip image](https://developer.android.com/images/reference/androidx/compose/material3/elevated-filter-chip-with-corner-morphing.png)
 *
 * 此筛选 Chip 应用了凸起样式。如需扁平样式，请使用 [FilterChip]。
 *
 * 点击筛选 Chip 会切换其选中状态。可以提供一个选中状态的 [前导图标]（例如对勾标记），显示在 Chip 标签的起始边缘。
 *
 * @param 已选择 此 Chip 是否被选中。
 ...
 * @param 修饰符 要应用于此 Chip 的 [Modifier]。
 * @param 已启用 控制此 Chip 的启用状态。当为 `false` 时，该组件不会响应用户输入，并且在外观上显示为禁用状态，同时无障碍服务也会将其识别为禁用。
 * @param 前导图标 Chip 起始位置的可选图标，位于 [标签] 文本之前。当 [已选择] 为 true 时，该图标可以视觉上指示 Chip
 * 已被选中（例如，通过对勾图标）。
 * @param 尾随图标 Chip 末尾的可选图标。
 * @param 颜色集 用于解析此 Chip 在不同状态下所用颜色的 [SelectableChipColors]。参见 [FilterChipDefaults.elevatedFilterChipColors]。
 * @param 阴影 用于解析此 Chip 在不同状态下海拔高度的 [SelectableChipElevation]。这控制 Chip 下方阴影的大小。
 * 此外，当容器颜色为 [ColorScheme.surface] 时，这控制作为主色叠加应用的数量。参见 [FilterChipDefaults.filterChipElevation]。
 * @param 边框 绘制在此 Chip 容器周围的边框。传入 `null` 表示无边框。参见 [FilterChipDefaults.filterChipBorder]。
 * @param 水平排列 Chip 子元素的横向排列方式。如果没有图标，则 [标签] 与 [边框] 之间的水平内边距
 * 将为 [内容内边距] 与此 [水平排列] 中间距的总和。
 * @param 内容内边距 此 Chip 内容周围的内边距，包括 [前导图标]、[标签] 和 [尾随图标]。
 * @param 交互源 一个可选的提升式 [MutableInteractionSource]，用于观察和发射此 Chip 的
...
fun 凸起过滤芯片(
    ...
    前导图标: @Composable (() -> Unit)? = null,
    尾随图标: @Composable (() -> Unit)? = null,
    颜色集: SelectableChipColors = FilterChipDefaults.tonalElevatedFilterChipColors(),
    阴影: SelectableChipElevation? = FilterChipDefaults.elevatedFilterChipElevation(),
    边框: BorderStroke? = null,
    水平排列: Arrangement.Horizontal =
        FilterChipDefaults.horizontalArrangement(前导图标 != null, 尾随图标 != null),
 ...
 * 或一系列邮件联系人。
 *
 * 输入 Chip 代表用户输入的离散信息片段。只要提供的形状是 [CornerBasedShape]，它还会根据 Chip 的状态在 [形状集]
 * 中提供的形状之间进行变形过渡。如果 [形状集] 中的某个形状不是 [CornerBasedShape]，那么 Chip 将根据用户交互在
 * [ChipShapes] 之间切换。
 *
 * ![Input chip image](https://developer.android.com/images/reference/androidx/compose/material3/input-chip-with-corner-morphing.png)
 *
 * 输入 Chip 可以在起始位置有一个前导图标或头像。如果两者都提供了，头像将优先显示。
 *
 * @param 已选择 此 Chip 是否被选中。
 * @param 单击回调 当此 Chip 被点击时调用。
 * @param 标签 此 Chip 的文本标签。
 ...
 * @param 修饰符 要应用于此 Chip 的 [Modifier]。
 * @param 已启用 控制此 Chip 的启用状态。当为 `false` 时，该组件不会响应用户输入，并且在外观上显示为禁用状态，同时无障碍服务也会将其识别为禁用。
 * @param 前导图标 Chip 起始位置的可选图标，位于 [标签] 文本之前。
 * @param 头像 Chip 起始位置的可选头像，位于 [标签] 文本之前。
 * @param 尾随图标 Chip 末尾的可选图标。
 * @param 颜色集 用于解析此 Chip 在不同状态下所用颜色的 [ChipColors]。参见 [InputChipDefaults.inputChipColors]。
 * @param 阴影 用于解析此 Chip 在不同状态下海拔高度的 [ChipElevation]。这控制 Chip 下方阴影的大小。此外，当容器颜色为
 * [ColorScheme.surface] 时，这控制作为主色叠加应用的数量。参见 [InputChipDefaults.inputChipElevation]。
 * @param 边框 绘制在此 Chip 容器周围的边框。传入 `null` 表示无边框。参见 [InputChipDefaults.inputChipBorder]。
 * @param 水平排列 Chip 子元素的横向排列方式。如果没有图标，则 [标签] 与 [边框] 之间的水平内边距将为 [内容内边距]
 * 与此 [水平排列] 中间距的总和。
 * @param 内容内边距 此 Chip 内容周围的内边距，包括 [前导图标]、[头像]、[标签] 和 [尾随图标]。
 * @param 交互源 一个可选的提升式 [MutableInteractionSource]，用于观察和发射此 Chip 的 [Interaction]。你可以使用
...
fun 输入芯片(
    ...
    头像: @Composable (() -> Unit)? = null,
    尾随图标: @Composable (() -> Unit)? = null,
    颜色集: SelectableChipColors = InputChipDefaults.tonalInputChipColors(),
    阴影: SelectableChipElevation? = InputChipDefaults.inputChipElevation(),
    边框: BorderStroke? = InputChipDefaults.inputChipBorder(已启用, 已选择),
    水平排列: Arrangement.Horizontal =
    ...
    内容内边距: PaddingValues =
        InputChipDefaults.contentPadding(头像 != null, 前导图标 != null, 尾随图标 != null),
    交互源: MutableInteractionSource? = null,
) =
    InputChip(
        selected = 已选择,
...
object 辅助芯片默认设置 { // AssistChipDefaults
    ...
    @Composable fun 辅助芯片颜色集() = AssistChipDefaults.assistChipColors()
    ...
    @Composable
    fun 辅助芯片阴影(
        阴影: Dp = AssistChipTokens.FlatContainerElevation,
        按压阴影: Dp = 阴影,
        聚焦阴影: Dp = 阴影,
        悬停阴影: Dp = 阴影,
        拖拽阴影: Dp = AssistChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = 阴影,
    ): ChipElevation =
    ...
    fun 辅助芯片边框(
        ...
        边框宽度: Dp = AssistChipTokens.FlatOutlineWidth,
    ): BorderStroke =
     ...
     *
     * @param 阴影 当 [AssistChip] 没有其他 [Interaction] 时使用的高度。
     * @param 按压阴影 当 [AssistChip] 被按下时使用的高度。
    ...
    @Composable
    fun 凸起辅助芯片阴影(
        阴影: Dp = AssistChipTokens.ElevatedContainerElevation,
        按压阴影: Dp = AssistChipTokens.ElevatedPressedContainerElevation,
        聚焦阴影: Dp = AssistChipTokens.ElevatedFocusContainerElevation,
        悬停阴影: Dp = AssistChipTokens.ElevatedHoverContainerElevation,
        拖拽阴影: Dp = AssistChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = AssistChipTokens.ElevatedDisabledContainerElevation,
    ): ChipElevation =
...
object 过滤芯片默认值 { // FilterChipDefaults
    ...
    @Composable
    fun 过滤芯片阴影(
        阴影: Dp = FilterChipTokens.FlatContainerElevation,
        按压阴影: Dp = FilterChipTokens.FlatSelectedPressedContainerElevation,
        聚焦阴影: Dp = FilterChipTokens.FlatSelectedFocusContainerElevation,
        悬停阴影: Dp = FilterChipTokens.FlatSelectedHoverContainerElevation,
        拖拽阴影: Dp = FilterChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = 阴影,
    ...
    fun 过滤芯片边框(
        ...
        边框宽度: Dp = FilterChipTokens.FlatUnselectedOutlineWidth,
        已选择边框宽度: Dp = FilterChipTokens.FlatSelectedOutlineWidth,
    ): BorderStroke =
    ...
    @Composable
    fun 凸起过滤芯片阴影(
        阴影: Dp = FilterChipTokens.ElevatedContainerElevation,
        按压阴影: Dp = FilterChipTokens.ElevatedPressedContainerElevation,
        聚焦阴影: Dp = FilterChipTokens.ElevatedFocusContainerElevation,
        悬停阴影: Dp = FilterChipTokens.ElevatedHoverContainerElevation,
        拖拽阴影: Dp = FilterChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = FilterChipTokens.ElevatedDisabledContainerElevation,
    ): SelectableChipElevation =
...
object 输入芯片默认值 { // InputChipDefaults
     ...
     * @param 按压阴影 此 Chip 在被按压时使用的阴影高度
    ...
    @Composable
    fun 输入芯片阴影(
        阴影: Dp = InputChipTokens.ContainerElevation,
        按压阴影: Dp = 阴影,
        聚焦阴影: Dp = 阴影,
        悬停阴影: Dp = 阴影,
        拖拽阴影: Dp = InputChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = 阴影,
    ): SelectableChipElevation =
    ...
    fun 输入芯片边框(
        ...
        禁用边框颜色: Color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.12f),
        禁用已选择边框颜色: Color = Color.Transparent,
        边框宽度: Dp = InputChipTokens.UnselectedOutlineWidth,
        已选择边框宽度: Dp = InputChipTokens.SelectedOutlineWidth,
    ): BorderStroke =
...
object 建议芯片默认值 { // SuggestionChipDefaults
    ...
    @Composable fun 建议芯片颜色集() = SuggestionChipDefaults.suggestionChipColors()
    ...
    @Composable
    fun 建议芯片阴影(
        阴影: Dp = SuggestionChipTokens.FlatContainerElevation,
        按压阴影: Dp = 阴影,
        聚焦阴影: Dp = 阴影,
        悬停阴影: Dp = 阴影,
        拖拽阴影: Dp = SuggestionChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = 阴影,
    ): ChipElevation =
    ...
    fun 建议芯片边框(
        ...
        边框颜色: Color = MaterialTheme.colorScheme.outlineVariant,
        禁用边框颜色: Color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.12f),
        边框宽度: Dp = SuggestionChipTokens.FlatOutlineWidth,
    ): BorderStroke =
    ...
    @Composable
    fun 凸起建议芯片阴影(
        阴影: Dp = SuggestionChipTokens.ElevatedContainerElevation,
        按压阴影: Dp = SuggestionChipTokens.ElevatedPressedContainerElevation,
        聚焦阴影: Dp = SuggestionChipTokens.ElevatedFocusContainerElevation,
        悬停阴影: Dp = SuggestionChipTokens.ElevatedHoverContainerElevation,
        拖拽阴影: Dp = SuggestionChipTokens.DraggedContainerElevation,
        禁用阴影: Dp = SuggestionChipTokens.ElevatedDisabledContainerElevation,
    ): ChipElevation =
...
val SelectableChipElevation.按压阴影: Dp
    get() = this.pressedElevation
...
/**
 * 表示可选 Chip 在不同状态下使用的容器颜色和内容颜色。
 *
 * @param 容器颜色 此 Chip 在已启用时的容器颜色。
 * @param 标签颜色 此 Chip 在已启用时的标签颜色。
 * @param 前导图标颜色 此 Chip 的起始图标在已启用时的颜色。
 * @param 尾随图标颜色 此 Chip 的末尾图标在已启用时的颜色。
 * @param 禁用容器颜色 此 Chip 在禁用时的容器颜色。
 * @param 禁用标签颜色 此 Chip 在禁用时的标签颜色。
 * @param 禁用前导图标颜色 此 Chip 的起始图标在禁用时的颜色。
 * @param 禁用尾随图标颜色 此 Chip 的末尾图标在禁用时的颜色。
 * @param 已选择容器颜色 此 Chip 在已选中且已启用时的容器颜色。
 * @param 禁用已选择容器颜色 此 Chip 在已选中但禁用时的容器颜色。
 * @param 已选择标签颜色 此 Chip 在已选中且已启用时的标签颜色。
 * @param 已选择前导图标颜色 此 Chip 的起始图标在已选中且已启用时的颜色。
 * @param 已选择尾随图标颜色 此 Chip 的末尾图标在已选中且已启用时的颜色。
 * @constructor 使用任意颜色创建实例，参见 [FilterChip] 中使用的默认颜色 [FilterChipDefaults.filterChipColors]
 * 和 [FilterChipDefaults.elevatedFilterChipColors]。
 */
fun 可选芯片颜色集(
    容器颜色: Color,
    标签颜色: Color,
...
val SelectableChipColors.容器颜色: Color
    get() = this.containerColor

val SelectableChipColors.标签颜色: Color
    get() = this.labelColor

val SelectableChipColors.前导图标颜色: Color
    get() = this.leadingIconColor

val SelectableChipColors.尾随图标颜色: Color
    get() = this.trailingIconColor

val SelectableChipColors.禁用容器颜色: Color
    get() = this.disabledContainerColor

val SelectableChipColors.禁用标签颜色: Color
    get() = this.disabledLabelColor

val SelectableChipColors.禁用前导图标颜色: Color
    get() = this.disabledLeadingIconColor

val SelectableChipColors.禁用尾随图标颜色: Color
    get() = this.disabledTrailingIconColor

val SelectableChipColors.已选择容器颜色: Color
    get() = this.selectedContainerColor

val SelectableChipColors.禁用已选择容器颜色: Color
    get() = this.disabledSelectedContainerColor

val SelectableChipColors.已选择标签颜色: Color
    get() = this.selectedLabelColor

val SelectableChipColors.已选择前导图标颜色: Color
    get() = this.selectedLeadingIconColor

val SelectableChipColors.已选择尾随图标颜色: Color
...
/**
 * 用于 Chips 中的形状。假设所有形状都是 [CornerBasedShape]，Chip 将根据其状态在这些形状之间进行变形过渡。
 *
 * @param 形状 是激活状态的形状。
...
/** 返回此 ChipShapes 的一个副本，可选择性地覆盖其中某些值。 */
fun ChipShapes.复制(
    形状: Shape? = this.shape,
...
val ChipShapes.形状: Shape
    get() = this.shape

val ChipShapes.已选择形状: Shape
    get() = this.selectedShape

val ChipShapes.按压形状: Shape
