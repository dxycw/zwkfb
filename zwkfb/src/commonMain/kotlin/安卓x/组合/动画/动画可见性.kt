...
import androidx.compose.animation.TransformScope
...
/**
 * [AnimatedVisibility] 可用于在 [Transition] 状态变化时，对其内容的显示和消失进行动画处理。
 *
 * [可见] 根据过渡状态 T 定义内容是否应该可见。
 *
 * [修饰符] 为容纳 [内容] 而创建的 [Layout] 的修饰符。
 *
 * [进入] 用于出现动画的进入过渡，默认同时垂直展开并淡入。
 *
 * [退出] 用于消失动画的退出过渡，默认同时垂直收缩并淡出。
 *
 * [可变转换] 用于控制在主过渡动画开始前、延迟阶段（例如预测性返回手势期间）的视觉变换代码块。仅当 [Transition]
 * 使用 [rememberTransition] 配合 [DeferredTransitionState] 创建时才会生效。默认值为 `null`，表示不应用手动变换。
 * 该阶段在调用 [DeferredTransitionState.defer] 时开始，在调用 [DeferredTransitionState.animateTo] 启动自动
 * 过渡时结束。在此阶段，可以手动操控内容的变换（如 [TransformScope.alpha] 和 [TransformScope.scale]）。这些变换会
 * 与过渡的初始状态**叠加**应用：alpha 和 scale 等属性以**乘法**方式组合，而 offset 以**加法**方式组合。未手动设置的
 * 属性默认使用过渡本身的值。
 * 过渡开始后，手动应用的变换将移交给配置的 [进入] 和 [退出] 过渡动画。对于退出内容，采用"**未指定则保持**"策略：如果
 * 指定了退出过渡（如 `fadeOut`），移交时会向该过渡的目标值进行动画；但如果未指定某个属性的退出过渡（如缺少 `slideOut`），
 * 该属性将保持最后的手动值，直到整个过渡完成。
 * 在延迟阶段期间，进入内容保持 [EnterExitState.PreEnter] 状态，退出内容保持 [EnterExitState.Visible] 状态。
 * @see EnterTransition
 * @see ExitTransition
 * @see fadeIn
 * @see expandIn
...
@Composable
public fun <T> DeferredTransition<T>.延迟动画可见性(
    可见: (T) -> Boolean,
    修饰符: Modifier = Modifier,
    进入: EnterTransition = fadeIn() + expandIn(),
    退出: ExitTransition = shrinkOut() + fadeOut(),
